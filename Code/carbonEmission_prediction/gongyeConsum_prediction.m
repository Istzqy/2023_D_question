...
%% 工业部门GDP预测灰色预测（短期预测10年内，弥补数据量不足缺陷）
...
gybmGdp_2 = [gybmGdp G(12:21)];
figure(3)
plot(t2,G,'o',t2,gybmGdp_2,'r--*')  %原始数据与预测数据的比较
xlabel('年份')
ylabel('Gdp')


time_series_data = gybmGdp_2' ;
...
subplot(2,1,2)
gy_gdp =  [train_data' forecast']';
plot(2021:2060,gy_gdp(12:51) , 'b--*', 'LineWidth', 1);
title('2021~2060工业消费部门GDP预测');
xlabel('时间');
ylabel('GDP(亿元)');
...
% 11. 区域工业部门GDP预测输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\gy_gdp.xlsx',gy_gdp);
...
% 15 代表的是 样本个数
tce = [14312.68 15171.86 15494.59 16347.81 17018.53	17242.19 17724.55 17832.32 18123.66	19109.55 18872.74]';
len = length(tce);
pelta = ones(len,1);
x = [pelta, prePopdata(1:11), gy_gdp(1:11)];

[b,bint,r,rint,stats]=regress(tce,x,0.05);     % 95%的置信区间

tce_NiHe = b(1) + b(2) .*  prePopdata(1:11) + b(3) .* gy_gdp(1:11)  ;
...
%% 岭回归预测
ridge_x= [prePopdata(1:11) gy_gdp(1:11)];
ridge_y = tce;
k = 0:0.1:10;
...
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\gongye_enePredata.xlsx',enePre);
