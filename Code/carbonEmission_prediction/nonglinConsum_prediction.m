...
%% 农林部门GDP预测灰色预测（短期预测10年内，弥补数据量不足缺陷）
...
yiGdp_2 = [yiGdp G(12:21)];
figure(3)
plot(t2,G,'o',t2,yiGdp_2,'r--*')  %原始数据与预测数据的比较
xlabel('年份')
ylabel('Gdp')


time_series_data = yiGdp_2' ;
...
subplot(2,1,2)
nonglin_gdp =  [train_data' forecast']';
plot(2021:2060,nonglin_gdp(12:51) , 'b--*', 'LineWidth', 1);
title('2021~2060农林消费部门GDP预测');
xlabel('时间');
ylabel('GDP(亿元)');
...
%11. 区域农林部门GDP预测输出
% xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\nonglin_gdp.xlsx',nonglin_gdp);

%% 多元线性回归分析

% 因为用的3是维拟合，则 x 应该为 3*15 的矩阵，第一列为 1 ，第二列为 x1 ，第三列为 x2 , 第四列为 x3
% 15 代表的是 样本个数
tce = [345.36 393.87 448.95	362.18	383.72	429.37	433.46	440.49	457.99	438.58	423.78]';
len = length(tce);
pelta = ones(len,1);
x = [pelta, prePopdata(1:11), nonglin_gdp(1:11)];

[b,bint,r,rint,stats]=regress(tce,x,0.05);     % 95%的置信区间

tce_NiHe = b(1) + b(2) .*  prePopdata(1:11) + b(3) .* nonglin_gdp(1:11)  ;

figure(2);
hold on;
plot(t1,prePopdata(1:11),'b--o');
plot(t1,nonglin_gdp(1:11),'r--o');
...
%% 岭回归预测
ridge_x= [prePopdata(1:11) nonglin_gdp(1:11)];
ridge_y = tce;
k = 0:0.1:10;
...
%% 2021~2060能源消费量预测
ridge_x1 = [prePopdata nonglin_gdp];
