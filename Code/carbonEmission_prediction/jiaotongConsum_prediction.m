...
%% 交通部门GDP预测灰色预测（短期预测10年内，弥补数据量不足缺陷）
...
jtbmGdp_2 = [jtbmGdp G(12:21)];
figure(3)
plot(t2,G,'o',t2,jtbmGdp_2,'r--*')  %原始数据与预测数据的比较
xlabel('年份')
ylabel('Gdp')


time_series_data = jtbmGdp_2' ;
...
subplot(2,1,2)
jt_gdp =  [train_data' forecast']';
plot(2021:2060,jt_gdp(12:51) , 'b--*', 'LineWidth', 1);
title('2021~2060交通消费部门GDP预测');
xlabel('时间');
ylabel('GDP(亿元)');
...
%区域交通部门GDP预测输出
% xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jt_gdp.xlsx',jt_gdp);
...
% 15 代表的是 样本个数
tce = [1398.26 1494.74 1618.15 1743.67 1915.88 2019.29 2083.59 2187.96 2324.65 2482.93 2484.47]';
len = length(tce);
pelta = ones(len,1);
x = [pelta, prePopdata(1:11), jt_gdp(1:11)];

[b,bint,r,rint,stats]=regress(tce,x,0.05);     % 95%的置信区间

tce_NiHe = b(1) + b(2) .*  prePopdata(1:11) + b(3) .* jt_gdp(1:11)  ;
...
%% 岭回归预测
ridge_x= [prePopdata(1:11) jt_gdp(1:11)];
ridge_y = tce;
k = 0:0.1:10;
...
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jiaotong_enePredata.xlsx',enePre);
