%%
%**代码功能：用于预测建筑部门的能源消费品种结构以及对应品种的碳排放因子
...
% 建筑部门能源消费预测，已经预测完成，直接进行数据读取
enePre = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jianzhu_enePredata.xlsx');
% 2010~2020建筑部门消费品种结构

meitan = [19.62	18.19	0.00	15.27	8.92	5.94	3.81	4.09	3.06	2.27	0.00]';
youpin = [81.64	109.82	144.33	112.79	92.76	76.34	50.09	35.60	33.71	34.34	11.25]';
tianran = [18.75	24.34	23.94	26.47	25.14	34.71	26.73	15.69	8.65	9.31	11.44]';
reli = [8.97	6.66	4.77	6.78	5.29	3.58	0.44	0.42	0.98	0.90	0.66]';
dianli = [405.59	461.82	517.77	576.90	596.21	636.25	713.88	805.45	929.75	992.64	999.80]';
...
%11. 区域建筑部门煤炭消耗预测输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_meitan_xiaohao.xlsx',meitan_xiaohao);
...
for i = 1:length(youpin_xiaohao)
    if(youpin_xiaohao(i)<0)
        youpin_xiaohao(i) = 0;
    end
end
plot(2021:2060,youpin_xiaohao(12:51) , 'b--*', 'LineWidth', 1);
title('2021~2060建筑消费部门油品消耗预测');
xlabel('时间');
ylabel('万tce');
...
%11. 区域建筑部门油品消耗预测输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_youpin_xiaohao.xlsx',youpin_xiaohao);
...
%11. 区域建筑部门天然气消耗预测输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_tianran_xiaohao.xlsx',tianran_xiaohao);
...
for i = 1:length(reli_xiaohao)
    if(reli_xiaohao(i)<0)
        reli_xiaohao(i) = 0;
    end
end
plot(2021:2060,reli_xiaohao(12:51) , 'b--*', 'LineWidth', 1);
title('2021~2060建筑消费部门热力消耗预测');
xlabel('时间');
ylabel('万tce');
%xlim([1, length(time_series_data) + num_steps]);
...
%11. 区域建筑部门热力消耗预测输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_reli_xiaohao.xlsx',reli_xiaohao);
...
%11. 区域建筑部门电力消耗预测输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_dianli_xiaohao.xlsx',dianli_xiaohao);
...
enePre = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jianzhu_enePredata.xlsx');
% 分别读取建筑部门各种能源的预测
meitan_xiaohao = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_meitan_xiaohao.xlsx');
youpin_xiaohao = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_youpin_xiaohao.xlsx');
tianran_xiaohao = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_tianran_xiaohao.xlsx');
reli_xiaohao = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_reli_xiaohao.xlsx');
dianli_xiaohao = xlsread('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jz_dianli_xiaohao.xlsx');
...
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jianzhu_anjiegou_xiaohao.xlsx',xiaohaofenbu);

%%  消费部门碳排放因子预测
% 思路：碳排放因子预测应与二次能源消耗比例相关联
% 对于建筑部门:主要体现在电力消耗比例提升对碳排放因子影响

meitan_yz = [2.664	2.664	2.664	2.664	2.664	2.664	2.664	2.664	2.664	2.664	2.664];
youpin_yz = [2.000	2.006	2.042	2.069	2.034	2.060	2.050	2.033	2.034	2.034	2.039];
...
legend('岭回归预测建筑部门煤炭的碳排放因子','Location','northwest');
subplot(3,2,2)
plot(t2,youpin_yz_pre,'B--o','LineWidth',1);
legend('岭回归预测建筑部门油品的碳排放因子','Location','northwest');
subplot(3,2,3)
plot(t2,dianli_yz_pre,'B--o','LineWidth',1);
legend('岭回归预测建筑部门电力的碳排放因子','Location','northwest');
subplot(3,2,4)
plot(t2,tianran_yz_pre,'B--o','LineWidth',1);
legend('岭回归预测建筑部门天然气的碳排放因子','Location','northwest');
subplot(3,2,5)
plot(t2,reli_yz_pre,'B--o','LineWidth',1);
legend('岭回归预测建筑部门热力的碳排放因子','Location','northwest');

%% 建筑部门碳排放量预测

jianzhu_tanpf_pre(:,1) = xiaohaofenbu(:,1).*meitan_yz_pre;
jianzhu_tanpf_pre(:,2) = xiaohaofenbu(:,2).*youpin_yz_pre;
jianzhu_tanpf_pre(:,3) = xiaohaofenbu(:,5).*dianli_yz_pre;
jianzhu_tanpf_pre(:,4) = xiaohaofenbu(:,3).*tianran_yz_pre;
jianzhu_tanpf_pre(:,5) = xiaohaofenbu(:,4).*reli_yz_pre;
jianzhu_tanpf_pre(:,6) = jianzhu_tanpf_pre(:,1)+jianzhu_tanpf_pre(:,2)+jianzhu_tanpf_pre(:,3)+jianzhu_tanpf_pre(:,4)+jianzhu_tanpf_pre(:,5) ;

figure(8)
plot(t2,jianzhu_tanpf_pre);
xlabel('年份');
ylabel('碳排放量(万tco2)');
title('建筑部门碳排放量预测');
legend({'煤炭碳排放','油品碳排放','电力碳排放','天然气碳排放','热力碳排放','建筑部门碳排放'});

% 建筑部门碳排放量预测数据输出
xlswrite('D:\FPGA_MATLAB_Learning\数学建模\结课报告\D题\Code\carbonEmission_prediction\jianzhu_tanpf_pre.xlsx',jianzhu_tanpf_pre);
